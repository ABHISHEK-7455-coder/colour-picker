...
const GradientMaker = () => {
  ...
  const addColorStop = () => {
    ...
    setColorStops([...colorStops, { id: Date.now(), position: Math.max(0, Math.min(100, lastPos / 2)), color: newColor }]);
    setNewColor('#000000');
  };
  ...
  const handleMouseDown = (id) => {
    ...
    const onMove = (e) => {
      ...
      let newP = ((e.clientX - rect.left) / rect.width) * 100;
      newP = Math.max(0, Math.min(100, newP));
      handlePositionChange(id, newP);
    };
    const onUp = () => {
  ...
  return (
    <div className="gradient-generator">
      ...
      <div className="grid">
        ...
        : gradientType === 'radial'
        ? `radial-gradient(circle, ${colorStops.map(s => `${s.color} ${s.position}%`).join(', ')})`
        : `conic-gradient(from ${angle}deg, ${colorStops.map(s => `${s.color} ${s.position}%`).join(', ')})` }}
          ></div>
